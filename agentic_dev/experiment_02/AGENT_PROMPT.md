...
### Why This Approach
- ✅ Experiment_02 is deployed and working
- ✅ Minimal scope reduces risk (3 tools vs 32)
- ✅ Same infrastructure = fast redeployment via `azd up`
- ✅ Incremental changes = easier debugging
- ⚠️ Auth configuration previously caused issues - track carefully
...
├── server.py                           # Main FastMCP server (has weather tools)
...
├── account/
│   ├── get-account.ts                  # Pattern for get_account tool
│   └── get-permissions.ts
├── chain/
│   ├── get-chain-info.ts               # Pattern for get_chain_info tool
│   └── get-block.ts                    # Pattern for get_block tool
└── [other categories - ignore for MVP]
...
├── package.json                        # @proton/cli v0.1.95
└── src/commands/                       # CLI command implementations
    ├── account/index.ts                # account <name>
    ├── chain/index.ts                  # chain
    └── block/index.ts                  # block <block_num>
...
├── CHANGES.md                          # ← Track every change here
├── AGENT_PROMPT.md                     # ← This file
└── mcp-server/                         # ← Copy experiment_02 here
    ├── server.py                       # ← Modify to use Proton CLI
    ├── host.json                       # ← Keep same
    ├── local.settings.json             # ← Keep same
    ├── requirements.txt                # ← Keep same (no new deps needed)
    ├── infra/
    │   ├── main.bicep                  # ← Update resource names
    │   └── main.parameters.json        # ← Update environment name
    └── azure.yaml                      # ← Keep same
```
...
```json
{
  ...
  "parameters": {
    "environmentName": {
      "value": "experiment-03"  // ← Change this from experiment_02
    },
    "location": {
      "value": "eastus2"  // ← Ensure this matches
    }
  }
}
```
...
#### `azure.yaml`
```yaml
name: xpr-mcp-server  # ← Update if needed
metadata:
...
### Phase 0: Setup & Verification ✅ (Documentation complete)
...
### Do NOT
- ❌ Implement all 32 tools from experiment_01 (only 3 for MVP)
- ❌ Modify infrastructure without documenting in CHANGES.md
- ❌ Change resource group from 'sama'
...
```
1. agent.md → Understand the vision
2. CHANGES.md → Understand what will be tracked
3. AGENT_PROMPT.md → Get implementation details (this file)
4. experiment_02/server.py → See working FastMCP server
5. experiment_01/src/tools/account/get-account.ts → See tool pattern
6. proton-cli/README.md → Understand CLI commands
```
